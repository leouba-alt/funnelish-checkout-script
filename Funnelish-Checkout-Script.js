...
// ============================================================
// ESPERA (SIN LÍMITE DE TIEMPO) A QUE UN CAMPO APAREZCA EN EL DOM
// ------------------------------------------------------------
// En computador, Funnelish suele pintar TODOS los campos del
// formulario de una sola vez. En celular, algunas tiendas/plantillas
// los pintan más tarde, o solo cuando el cliente avanza a ese paso
// del formulario (checkout por pasos) — y eso puede tardar más de
// unos pocos segundos, según el celular, la conexión o si el cliente
// se toma su tiempo llenando el formulario.
//
// En vez de reintentar un número fijo de veces y rendirse, esta
// función se queda "vigilando" los cambios en la página (usando
// MutationObserver) y actúa en el instante en que el campo aparece,
// sin importar cuánto tarde. Deja de vigilar apenas lo encuentra.
// ============================================================
function waitForElement(buscarElemento, alEncontrarlo, opciones = {}) {
    ...
    const avisoTrasMs = opciones.avisoTrasMs || 8000; // aviso informativo (no deja de esperar)
    let resuelto = false;
    let observer = null;
    let avisoTimer = null;
    ...
    // Intento inmediato, por si el campo ya existe (caso normal en computador)
    if (intentar()) return;

    // Aviso informativo si está tardando -- NO deja de esperar, solo avisa
    avisoTimer = setTimeout(() => {
        if (!resuelto) {
            console.warn(`[Integramelo] "${descripcion}" todavía no aparece en la página después de ${(avisoTrasMs / 1000).toFixed(1)}s. Se sigue esperando en segundo plano (normal si el formulario está dividido en varios pasos, o el celular/conexión es más lento).`);
        }
    }, avisoTrasMs);

    // Vigilamos cualquier cambio en la página y reintentamos cada vez,
    // sin límite de tiempo, hasta que el campo aparezca.
...
// ============================================================
// ELEGIR EL CAMPO VISIBLE CUANDO HAY VARIOS CON EL MISMO NOMBRE
// ------------------------------------------------------------
// Algunas plantillas de Funnelish dibujan el formulario DOS VECES
// en el mismo HTML (una versión pensada para pantallas grandes y
// otra para celular) y usan CSS para mostrar solo la que corresponde
// según el tamaño de pantalla -- pero AMBAS existen en la página al
// mismo tiempo. Si buscamos un campo por su nombre y nos quedamos
// con el primero que aparece en el HTML, en el celular podemos
// terminar leyendo/llenando el que está oculto, mientras el que el
// cliente sí ve se queda vacío. Esta función revisa la lista de
// candidatos y devuelve el que realmente está visible en pantalla.
// ============================================================
function getVisibleElement(elementos) {
    for (const el of elementos) {
        if (el && el.offsetParent !== null) {
            return el;
        }
    }
    // Si ninguno parece visible todavía (ej. la página apenas está pintando),
    // devolvemos el primero que exista, para no dejar todo en blanco.
...
function personalizarSelects() {
    ...
    waitForElement(
        () => {
            const stateSel = getVisibleElement(document.getElementsByName("shipping_state"));
            const citySel = getVisibleElement(document.getElementsByName("shipping_city"));
...
function ocularEmail() {
    const inputEmail = getVisibleElement(document.getElementsByName("email"))
    ...
    waitForElement(
        ...
        (submitButton) => {
            ...
            submitButton.addEventListener('click', function (event) {
                var inputTelefono = getVisibleElement(document.getElementsByName('phone'));
...
function recoverData() {
    // Usamos getVisibleElement porque algunas tiendas dibujan el formulario dos
    // veces (una oculta para escritorio, otra visible para celular) -- así nos
    // aseguramos de leer siempre lo que el cliente realmente ve y llenó.
    const getValueQuery = (name) => getVisibleElement(document.getElementsByName(name))?.value || '';
...
waitForElement(
    () => getVisibleElement(document.querySelectorAll(`[data-name="LeadID"]`)),
    (leadIdField) => {
...
function ocultarInputs() {
    ...
    // tienda/dispositivo (ej. móvil) tarda un poco más en aparecer.
    waitForElement(
        () => getVisibleElement(document.getElementsByName("shipping_country")),
        (countryField) => {
